...
> The front door: author, validate, deploy, and run workflows. MIT. Public in **Phase 1**.
>
...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init [dir]`        | `--template <name>` (default: the built-in `hello` template), `--python`                                                                                                | Scaffold a workflow package from an `examples` template: `workflow.jsonc`, program file, `README.md`, `package.json`, `tsconfig.json`, `.gitignore`. Writes the package ONLY — no vendored agent skills (they ship in the plugin, which upgrades; a scaffolded copy would freeze) |
...
| `build <file>`      | `--out <path>` (default `<workflow-name>.mjs`)                                                                                                                          | Bundle to one deployable `.mjs` — `@boardwalk-labs/workflow` left external, `meta` intact — for a self-hosted server's `BOARDWALK_WORKFLOWS_DIR`                                                                                                                                  |
